...
export const SuperStackedMargin =
  -1 * (CardWidth + 2 * CardMargin + 2 * padding + 4);
...
export const Card: React.FC<CardProps> = (props) => {
  ...
  const onDragStart = React.useCallback<GameDragEvent>(
    (ev) => {
      ev.persist();
...
const CardFace: React.FC<{
  ...
  const image = deck_images.get(suit)![raw];
...
export const CardBack: React.FC<{ back?: Back; width: number }> = (props) => {
  ...
  return <img alt='Back of a card' src={backCard} width={width} />;
};
