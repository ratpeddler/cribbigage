...
export const Throw: GameComponent = (props) => {
  ...
  const selectedLength = Object.keys(keepCards).filter(
    (key) => !!keepCards[key as any]
  ).length;
  let disabled: boolean = selectedLength !== keepSize;

  const score = scoreHand(
    user.hand.filter((c) => keepCards[c]),
    []
  );
      ...
      userActions={() => (
        <div
          ...
          <h3 style={{ textAlign: "center" }}>
            Select which cards you will keep and which you will discard to{" "}
            <span style={{ color: dealer.color }}>
              {yourCrib ? "your" : dealer.name + "'s"} crib
            </span>
            .
            <div>
              (<span style={{ color: user.color }}>You</span> must keep{" "}
              {keepSize} cards)
            </div>
          </h3>
          {hasConfirmed ? (
            <Button
              ...
              onClick={props.refreshFromServer}>
            ...
            <Button
              ...
              onClick={() => {
                if (
                  LocalOrMultiplayer == "online" &&
                  (!isYourTurn || !!props.waitingForServer)
                ) {
              ...
              {disabled
                ? `Select ${keepSize - selectedLength} more cards`
